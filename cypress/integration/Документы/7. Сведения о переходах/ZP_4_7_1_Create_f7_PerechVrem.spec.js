...
describe('4.7.1. Создание постоянного перехода', ()=>{
it('4.7.1. Создание постоянного пакетного переходов', ()=>{
    ...
    //Заполнить поля формы: дата Начало, дата Окончание
    ...
    cy.wait(5000)
    cy.get('#idi_SCREENPARAMETERS_RETURNDATE .dx-texteditor-input-container').type('{selectall}{backspace}').type('25042022') //установить дату Окончание
    cy.wait(5000)
   
    //Заполнение поля Подразделение
    cy.get('[ng-reflect-ng-switch="SelectBox"] #idi_SCREENPARAMETERS_DEPARTMENTQUALIFIEDNAME').click() //открыть справочник Подразделений
    cy.wait(10000)
    cy.get('[ng-reflect-ng-switch="SelectBox"] #idi_SCREENPARAMETERS_DEPARTMENTQUALIFIEDNAME').click() //открыть справочник Подразделений
    cy.wait(10000)
    ...
    cy.get(' [aria-label="Выбрать"] .dx-button-text').click({force: true})
    cy.wait(3000)
